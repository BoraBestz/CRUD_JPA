...
import com.esign.service.configuration.service.pass.PasswordHistoryService;
...
public class PasswordHistoryController {
    ...
    @Autowired
    public void setLanguageController(PasswordHistoryService passwordHistoryService,
    ...
    @ApiOperation(value = "Search Password History", response = PasswordHistoryDto.class, responseContainer = "List")
    @ApiResponses(
            value = {
    ...
    public ResponseEntity<ResponseModel> searchByCri(
            ...
            @RequestParam(required = false, defaultValue = "passwordHistory", value = "sort") String sort,
        ...
        try {
            log.info("Start PasswordHistoryController.searchByCri()... ");
            PasswordHistoryDto dto = modelMapper.map(paramDto, PasswordHistoryDto.class);
            Page<PasswordHistoryDto> byCri = passwordHistoryService.findByCri(dto);
            ...
            log.error("Error PasswordHistoryController.searchByCri()... " + e.getMessage());
            ...
            loggingServiceClient.activityLog(
                    ...
                    status);
            // ***END ACTIVITY LOG***//
            log.info("End PasswordHistoryController.searchByCri()... ");
        }
    }

    //@PreAuthorize("hasAnyAuthorityCustom('ARMTV')")
    @PreAuthorize("isAuthenticated()")
    @GetMapping("/password/history/{id}")
    @ApiOperation(value = "Get Password History By Id", response = PasswordHistoryDto.class)
    @ApiResponses(
            value = {
                    @ApiResponse(code = 400, message = "Something went wrong"),
                    @ApiResponse(code = 403, message = "Access denied"),
    ...
    public ResponseEntity<ResponseModel> getById(@PathVariable("id") int id) {
        ...
        /// FOR LOG///
        try {
            log.info("Start PasswordHistoryController.getById()... ");
            PasswordHistoryDto dto = passwordHistoryService.getById(id);

            if (dto.getPasswordHistoryId() > 0) {
                /// FOR LOG///
                status = HttpStatus.OK.value();
                remark = "ID=" + id;
                /// FOR LOG///
            ...
            log.error("Error PasswordHistoryController.getById()... " + e.getMessage());
            ...
            loggingServiceClient.activityLog(
                    ...
                    status);
            // ***END ACTIVITY LOG***//
            log.info("End PasswordHistoryController.getById()... ");
        }
    }

    @PreAuthorize("isAuthenticated()")
    @PostMapping("/password/history")
    @ApiOperation(value = "New Password History", response = void.class)
    @ApiResponses(
            value = {
                    @ApiResponse(code = 400, message = "Something went wrong"),
                    @ApiResponse(code = 403, message = "Access denied"),
    ...
    public ResponseEntity<ResponseModel> addConfig(@RequestBody PasswordHistoryDto dto){
        //FOR LOG//
        int status = 0;
        String remark = "";
        //FOR LOG//
        try {
            int count = passwordHistoryService.checkDuplicate(dto);
            if (count > 0) {
                throw new BusinessServiceException(
            ...
            PasswordHistoryEntity save = passwordHistoryService.save(dto);

            /// FOR LOG///
            status = HttpStatus.OK.value();
            remark = "ID=" + save.getPasswordHistoryId() + " NAME=" + save.getPasswordHistory();
            /// FOR LOG///

            return HelperUtils.responseSuccess(save.getPasswordHistoryId());
        } catch (BusinessServiceException e) {
            log.error("Error PasswordHistoryController.create()... " + e.getMessage());

            /// FOR LOG///
            status = e.getHttpStatus().value();
            remark = e.getMessage() + " NAME=" + dto.getPasswordHistory();
            /// FOR LOG///
            ...
            loggingServiceClient.activityLog(
                    ...
                    status);
            // ***END ACTIVITY LOG***//
            log.info("End PasswordHistoryController.create()... ");
        }
    }

    //@PreAuthorize("hasAnyAuthorityCustom('ARMTE')")
    @PreAuthorize("isAuthenticated()")
    @PutMapping("/password/history")
    @ApiOperation(value = "Update Password History", response = void.class)
    @ApiResponses(
            value = {
                    @ApiResponse(code = 400, message = "Something went wrong"),
                    @ApiResponse(code = 403, message = "Access denied"),
    ...
    public ResponseEntity<ResponseModel> update(@RequestBody PasswordHistoryDto dto) {
        /// FOR LOG///
        int status = 0;
        String remark = "";
        PasswordHistoryDto dtoFromDb = null;
        PasswordHistoryDto dtoFromDbCurrentData = null;
        /// FOR LOG///
        try {
            log.info("Start PasswordHistoryController.update()... ");
            dtoFromDb = passwordHistoryService.getById(dto.getPasswordHistoryId());
            ...
            if (dtoFromDb.getPasswordHistoryId() > 0) {
                if (dto.getPasswordHistory() != null
                        && !dtoFromDb.getPasswordHistory().equalsIgnoreCase(dto.getPasswordHistory())) {
                    int count = passwordHistoryService.checkDuplicate(dto);
                    if (count > 0) {
                        throw new BusinessServiceException(
                                HttpStatus.CONFLICT, message.getMessage("ERM0002"), "ERM0002");
                ...
                passwordHistoryService.save(dtoFromDb);
            ...
            log.error("Error PasswordHistoryController.update()... " + e.getMessage());

            /// FOR LOG///
            status = e.getHttpStatus().value();
            remark = e.getMessage() + " ID=" + dto.getPasswordHistoryId();
            /// FOR LOG///
            ...
            log.info("End PasswordHistoryController.update()... ");
        }
    }

    //@PreAuthorize("hasAnyAuthorityCustom('ARMTD')")
    @PreAuthorize("isAuthenticated()")
    @DeleteMapping("/password/history/{id}")
    ...
    public ResponseEntity<ResponseModel> delete(@PathVariable("id") int id) {
        ...
        /// FOR LOG///
        try {
            log.info("Start PasswordHistoryController.delete()... ");
            PasswordHistoryDto dtoFromDb = passwordHistoryService.getById(id);
            if (dtoFromDb != null) {
            ...
            log.error("Error PasswordHistoryController.delete()... " + e.getMessage());
            ...
            loggingServiceClient.activityLog(
                    ...
                    status);
            // ***END ACTIVITY LOG***//
            log.info("End PasswordHistoryController.delete()... ");
        }
    }
